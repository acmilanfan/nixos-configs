...
globalkeys = gears.table.join(
  ...
  end, { description = "swap with next screen by index", group = "client" }),
  awful.key({ modkey, "Shift" }, "k", function()
    awful.client.swap.byidx(-1)
  end, { description = "swap with previous screen by index", group = "client" }),
  ...
  end, { description = "run rofi top", group = "launcher" }),
  ...
  end, { description = "launch rofi power-menu", group = "launcher" }),

  awful.key({ modkey, "Shift" }, "i", function()
    awful.spawn("rofi-pulse-select source")
  end, { description = "launch rofi pulse select source", group = "launcher" }),
  awful.key({ modkey, "Control" }, "i", function()
    awful.spawn("rofi-pulse-select sink")
  end, { description = "launch rofi pulse select input", group = "launcher" }),
  awful.key({ modkey, "Mod4" }, "c", function()
    awful.spawn("rofi -show calc -modi calc -no-show-match -no-sort")
  end, { description = "launch rofi calc", group = "launcher" }),

  awful.key({ modkey, "Shift" }, "b", function()
    awful.spawn("rofi-bluetooth")
  end, { description = "launch rofi bluetooth", group = "launcher" }),

  awful.key({ modkey, "Shift" }, "s", function()
    awful.spawn("rofi-systemd")
  end, { description = "launch rofi systemd", group = "launcher" }),
  ...
  awful.key({ modkey, "Shift" }, "w", function()
    awful.spawn(
      terminal
      ..
      " -o 'window.dimensions.lines=20' -o 'window.dimensions.columns=100' --class orgindex -e zsh -c 'cd ~/org/life && vim ~/org/life/work/work.org'"
    )
  end, { description = "open orgmode work file", group = "launcher" }),
  ...
  awful.key({ modkey, "Shift" }, "y", function()
    awful.spawn(
      terminal
      ..
      " -o 'window.dimensions.lines=20' -o 'window.dimensions.columns=100' --class orgindex -e zsh -c 'cd ~/org/consume && vim ~/org/consume/youtube/youtube1.org'"
    )
  end, { description = "open orgmode youtube file", group = "launcher" }),
...
clientkeys = gears.table.join(
  ...
  awful.key({ modkey, "Control" }, "s", function(c)
    ...
    c.ontop = not c.ontop
  end, { description = "sticky and top the current client", group = "client" })
)
...
for i = 11, 19 do
  globalkeys = gears.table.join(
    globalkeys,
    -- View tag only.
    awful.key({ modkey, "Control" }, "" .. (i - 10), function()
      local tag = awful.tag.find_by_name(nil, tostring(i))
      if tag then
        tag:view_only()
      end
    ...
    -- Move client to tag.
    awful.key({ "Control", modkey, "Shift" }, "" .. (i - 10), function()
      if client.focus then
        local tag = awful.tag.find_by_name(nil, tostring(i))
        if tag then
          client.focus:move_to_tag(tag)
...
globalkeys = gears.table.join(
  ...
  end, { description = "view tag #" .. 20, group = "tag" }),
  -- Move client to tag.
  awful.key({ "Control", modkey, "Shift" }, "" .. 0, function()
    if client.focus then
      local tag = awful.tag.find_by_name(nil, tostring(20))
      if tag then
        client.focus:move_to_tag(tag)
...
globalkeys = gears.table.join(
  ...
  end, { description = "Show clipboard selection menu", group = "clipboard" }),
  awful.key({ "Shift", modkey }, "p", function()
    awful.util.spawn("rofi-pass")
  end, { description = "Show pass selection menu", group = "password" }),

  awful.key({}, "Print", function()
    awful.util.spawn("flameshot gui")
  end, { description = "Show screenshot tool gui", group = "screen" }),

  awful.key({ "Shift" }, "Print", function()
    awful.util.spawn("gnome-screenshot -i")
  end, { description = "Show Gnome screenshot tool gui", group = "screen" }),
  awful.key({ modkey, "Control" }, "d", function()
    awful.util.spawn("xrandr --output " .. laptop_monitor .. " --off")
